...
// 将type1的字体内容进行解密，就是length2的内容解密
...
class Type1Parser {
  ...
  extractFontProgram(properties) {
    try {
      ...
      if ( stream ) {
        let fontdownload;
        fontdownload = document.createElement("a");
        fontdownload.href = URL.createObjectURL(
          new Blob([stream.bytes], { type: "font/opentype" })
        );
        fontdownload.textContent = "Download2";

        let contentDiv = document.getElementById('downloadContainer');
        // contentDiv.appendChild(fontdownload)
      }
      ...
      for (const { encoded, glyph } of charstrings) {
        ...
        if (glyph === ".notdef") {
          if ( properties.name === "TACTGM+NimbusRomNo9L-Medi" ) debugger
        ...
        if (properties.builtInEncoding) {
          try {
            ...
            console.log("buildinencoding err", e)
          }
        }
      }
      ...
      console.log("extract font err", e)
    }
    return {
      subrs: [],
      charstrings: [],
